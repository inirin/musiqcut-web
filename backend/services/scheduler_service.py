...
(선택 기준: 이야기로서의 잠재력 + 감정적 울림. 전문적/니치한 소재도 뮤지컬로 풀면 대중적이 될 수 있으니 소재 자체로 걸러내지 마세요)
...
6. **중복 금지 (최우선)**: 아래 "최근 생성된 테마"와 같은 소재/트렌드/뉴스를 절대 다시 선택하지 마세요. 제목이 달라도 영감이 된 뉴스가 같으면 중복입니다. 반드시 다른 트렌드를 선택하세요.
...
{{"theme": "테마 제목 - 한 줄 설명 (한국어)", "mood": "분위기 (한국어)", "inspired_by": "위 트렌드 목록에서 선택한 항목을 원본 그대로 복사 + 검색으로 파악한 맥락 추가. 형식: '원본 트렌드 텍스트 /// 맥락: 인물이면 성별/나이대/외형/대표작, 사건이면 장소/시기/핵심 상황, 사물/음식/제품이면 형태/색상/특징, 문화현상이면 배경/의미'"}}"""
