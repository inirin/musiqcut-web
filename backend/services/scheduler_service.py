...
3. **뉴스 속 구체적 숫자·장소·행동·상황을 테마 제목과 설명에 반드시 포함** — 이것이 가장 중요!
4. 제목만 읽어도 "이 뉴스다!" 하고 10명 중 7명이 맞출 수 있어야 함
5. 장르는 자유 (판타지, SF, 코미디, 로맨스, 드라마, 풍자, 호러 등)

좋은 예시 (구체적 디테일이 제목에 녹아있음):
- "SK하닉 3.71% 하락, 터보퀀트 충격" → "3비트 마법사의 폭풍 - 16비트 기억을 3비트로 압축하자 반도체 거인들이 하루아침에 무너지던 날"
  (3비트/16비트/반도체/하락 → 터보퀀트 뉴스 즉시 연상)
- "독도 영유권 주장, 교과서 왜곡" → "작은 섬의 큰 거짓말 - 이웃 나라가 교과서에서 '강제'를 지우고 '바다 위 섬'을 자기 것이라 쓴 날"
  (교과서/강제 삭제/섬 영유권 → 독도 뉴스 즉시 연상)
- "풍력발전기 화재, 80미터 추락" → "80미터 하늘의 불꽃 - 바닷가 풍차 위에서 시작된 불, 나흘째 타오르는 날개"
  (80미터/풍차/바닷가/나흘째 → 영덕 풍력 화재 즉시 연상)

나쁜 예시 (추상적 — 절대 이렇게 하지 마세요):
- "기억 압축의 멜로디" — 어떤 뉴스인지 전혀 짐작 불가
- "춤추는 알고리즘의 파도" — 멋있지만 아무도 뉴스를 떠올릴 수 없음
- "보이지 않는 코드 한 줄" — 너무 일반적
