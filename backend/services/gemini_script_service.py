...
(분위기에 [트렌드 힌트: ...]가 포함되어 있다면, 그 대상(인물/사물/음식/제품/작품/브랜드/장소 등)을 Google 검색으로 정확히 파악하세요.
 - 캐릭터/소재의 **실제 외형**을 Google 검색으로 파악:
   · 인물이면: 얼굴형, 눈매, 코, 체형, 헤어, 의상 스타일 → 닮은 애니메이션 캐릭터로 변환
   · 사물/음식/제품이면: 실제 형태, 색상, 질감, 크기, 특징적 디테일 → 작품의 소재/배경/소품에 정확히 반영
   · 장소/건물이면: 실제 외관, 구조, 분위기 → 배경 묘사에 반영
   (반드시 "animated" 스타일로 표현, 실사/사진풍 절대 금지)
 - vocal_style: 트렌드 대상에 어울리는 보컬 스타일 반영
 - art_style: 트렌드 대상의 **실제 비주얼 톤/분위기**를 반영
 - music_prompt: 트렌드 대상과 연관된 **음악 장르/분위기**를 반영
...
- **가사만 들어도 테마의 소재/상황이 구체적으로 그려져야 함** — 추상적 은유만 나열하지 말고, 소재의 핵심 장면/행동/감정이 드러나도록. 표현 방식은 자유 (감각적, 대화체, 아이러니, 직접 묘사 등)
- **진부한 단어 반복 금지**: "그림자", "빛", "노래", "멜로디", "하늘", "바람", "별", "꿈" 등을 매번 넣지 마세요. 테마 고유의 구체적인 단어를 사용하세요
...
- **[트렌드 힌트]가 있으면**: 해당 작품/브랜드의 **실제 비주얼 톤**을 검색하여 art_style에 반영
- 2D, 3D, 혼합, 실험적 스타일 등 자유롭게 — 하이브리드나 독창적 스타일도 환영
...
해시태그 (반드시 한국어로 작성):
- 이 작품의 검색 노출용 한국어 해시태그 5개 생성
- 작품 내용을 잘 드러내는 고유 키워드 (범용/고유명사 금지)
- 한국어 단일 단어 위주, 최대 2단어, 언더바 금지
...
- **[트렌드 힌트]가 있으면**: 트렌드 대상을 Google 검색하여 실제 외형을 파악하고 캐릭터/소품/배경에 반영:
  · 인물: 얼굴형, 눈매, 코, 헤어, 체형, 의상 → 닮은 애니메이션 캐릭터로 변환
  · 사물/음식/제품: 형태, 색상, 질감, 디테일 → image_prompt 소품/배경에 정확히 반영
  · 단, **반드시 2D/3D 애니메이션 캐릭터로 표현** — 실사/사진풍 묘사 절대 금지. "animated character"를 description_en 맨 앞에 명시
...
- **[트렌드 힌트]가 있으면**: 해당 대상에 어울리는 보컬 스타일을 검색하여 vocal_style과 music_prompt에 반영
- vocal_style 필드에 한국어로 간결하게 명시 (성별/나이 + 톤/스타일 — 테마와 장르에 맞게 자유롭게)
- music_prompt에도 영문으로 보컬 특성을 포함 — 반드시 vocal_style과 일관되게

제목 작성 규칙:
- 제목 스타일은 자유롭게 — 직접적, 시적, 아이러니, 질문형, 대화체, 함축적 1단어 등 매번 다르게
- "그림자", "빛", "노래", "멜로디", "하늘", "바람", "별", "꿈" 등 진부한 단어를 제목에 반복 사용하지 마세요. 테마 고유의 구체적인 단어를 사용하세요
{recent_titles_block}
...
{{
  ...
  "vocal_style": "보컬 스타일 (한국어, 성별/나이 + 톤/스타일 — 반드시 주인공 캐릭터와 일치)",
  "art_style": "선택한 애니메이션 아트 스타일 (영문, 구체적으로 — photorealistic/photograph 금지)",
  "characters": [
    {{
      "name": "캐릭터 이름/별칭 (한국어)",
      "description_en": "animated character, [성별], [나이대], [헤어 상세], [눈], [피부], [체형 상세], [의상], [액세서리] — 테마에 맞는 캐릭터를 자유롭게 설계"
    }}
  ],
  "hashtags": ["#해시태그1", "#해시태그2", "...정확히 5개"]
}}
...
- **와이드샷에 인물이 없는 장면**: 실내/실외 모두 가능. 빈 거리, 건물 외관, 실내 풍경, 자연, 소품 클로즈업 등 자유롭게 구성
- 가장 감정이 강한 가사 줄은 클로즈업으로 배치
- **보컬 구간(is_vocalist=true)의 클로즈업/미디엄샷은 반드시 주인공 1명만** 배치. 다른 인물이 함께 등장하면 안 됨 (AI 립싱크 모델이 1인 전용이라 여러 명이면 실패)
...
HASHTAG_PROMPT = """아래 작품 정보를 보고 YouTube/TikTok/Instagram 숏폼 업로드용 해시태그를 정확히 5개 생성하세요.
반드시 JSON 형식으로만 응답하세요.
...
해시태그 규칙 (반드시 한국어로 작성):
- 작품 내용을 잘 드러내는 한국어 고유 키워드 5개 (범용/고유명사 금지)
- 한국어 단일 단어 위주, 최대 2단어, 언더바 금지
