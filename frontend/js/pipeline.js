...
function _addRetryButton(stepEl, step) {
  ...
  wrap.style.cssText = 'margin-top:12px; padding-top:10px; border-top:1px dashed var(--border);';
  const btn = document.createElement('button');
  btn.className = 'btn btn-primary btn-sm';
  btn.textContent = `↺ STEP ${step}부터 재시도`;
  btn.onclick = (e) => { e.stopPropagation(); if (confirm(`STEP ${step}부터 재시도하시겠습니까?`)) retryFromStep(window._currentProjectId, step); };
